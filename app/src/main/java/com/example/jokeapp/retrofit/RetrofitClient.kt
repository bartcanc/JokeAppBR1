...
// The RetrofitClient object is a singleton class that creates a Retrofit
// instance to access the API
...
interface ApiService {
    // GET request to fetch the Categories from the API. Call is used to make the network request
// asynchronously and return the response in a callback
    @GET("categories")
    fun getCategories(): Call<CategoriesResponse>
    // GET request to fetch the Jokes from the API based on the
// category, amount and optional blacklist flags, and type fields (marked with ? operator)
// Category is part of the URL path (@Path annotation), amount is a query (@Query) parameter,
// and blacklist flags and type are optional query parameters.
// @Query parameters are used to pass
// key-value pairs in the URL query string e.g. ?amount=value1&type=value2
// The response is either a list of Jokes or an Error wrapped in a JokesResponse object
// retrieved asynchronously with a Call
// Examples:
